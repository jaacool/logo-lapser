...
const performAlignment = (
    ...
    // KeyPointVectors are returned, so their lifecycle must be managed by the caller.
    // We do not add them to the `mats` array for automatic cleanup here.
    ...
    try {
        ...
        let affineTransform = cv.estimateAffine2D(matTargetPts, matBasePts, new cv.Mat(), cv.RANSAC);
        
        if (affineTransform.empty()) {
            throw new Error("Could not compute the initial transformation.");
        }

        // --- Pass 2: Iterative Refinement (Optional) ---
        if (useRefinement) {
            ...
            const keypointsRefined = new cv.KeyPointVector(); mats.push(keypointsRefined);
            ...
            if (descriptorsRefined.rows > 0) {
                ...
                if (goodMatchesRefined.length >= MIN_MATCH_COUNT) {
                    ...
                    if (!affineRefinement.empty()) {
                        ...
                        affineTransform.delete(); // delete old one
                        affineTransform = finalAffine; // assign new combined one
                    }
                }
            }
        }
        return { affineTransform, keypointsBase, keypointsTarget, goodMatches };

    } catch (e) {
        // If an error occurs, we must manually clean up the keypoint vectors
        // as they are not in the `mats` array.
...
export const processImageLocally = (
    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            let affineTransform: any;
            let debugUrl: string;

            if(isMaster) {
                // For the master image, create an identity transform
                affineTransform = cv.matFromArray(2, 3, cv.CV_64FC1, [1, 0, 0, 0, 1, 0]); mats.push(affineTransform);
                // Create a blank canvas for debug URL as there's no matching
                const dummyCanvas = document.createElement('canvas');
                dummyCanvas.width = 1; dummyCanvas.height = 1;
                debugUrl = dummyCanvas.toDataURL();
            } else {
                 const alignResult = performAlignment(masterMat, targetMat, isGreedyMode, isRefinementEnabled);
                 affineTransform = alignResult.affineTransform; // This mat will be deleted later
                 mats.push(affineTransform);
                 ...
                 // Now that we are done with them, delete the keypoint vectors
            ...
            cv.warpAffine(targetMat, warpedTarget, affineTransform, dsize, cv.INTER_LINEAR, cv.BORDER_REPLICATE);
...
export const refineWithGoldenTemplate = async (
    ...
    try {
        ...
        // Perform alignment: target -> template. Use stricter settings for refinement.
        const alignResult = performAlignment(templateMat, targetMat, false, false);
        const affineTransform = alignResult.affineTransform; mats.push(affineTransform);
        ...
        // Clean up the returned keypoint vectors after use
        alignResult.keypointsBase.delete();
        alignResult.keypointsTarget.delete();
